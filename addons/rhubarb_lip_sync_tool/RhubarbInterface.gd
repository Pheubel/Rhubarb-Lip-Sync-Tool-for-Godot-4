...
const output_directory := "res://.godot/rhubarb/output"
const dialog_directory := "res://.godot/rhubarb/dialog"
...
func run_rhubarb(input_file: String, dialog_file: String = "", recognizer: String = "pocketSphinx", output_format: String = "tsv") -> String:
	...
	var output_path = output_directory.path_join("%s%s" % [input_file.get_file().trim_suffix(input_file.get_extension()), output_format])
...
func _ensure_directories_exist() -> void:
	...
	if !DirAccess.dir_exists_absolute(dialog_directory):
		DirAccess.make_dir_recursive_absolute(dialog_directory)
